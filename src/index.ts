...
export default class Axioma {
  ...
  private parseGeneratedOutput(generatedContent: string): GeneratedOutput | null {
    try {
  ...
  private detectContentTypeFallback(content: string): { type: string; extension: string } {
  ...
  private formatCodeFallback(content: string, type: string): string {
  ...
  private suggestFilenameFromContent(code: string, type: string): string {
  ...
  async generateCodeFromTZ(projectPath: string = ".", outputFile?: string): Promise<void> {
    try {
      const fs = await import('fs');
      const path = await import('path');
      
      const tzFiles: string[] = [];
      
      function findTZFiles(dir: string) {
