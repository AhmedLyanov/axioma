...
export default class Axioma {
  ...
  private parseMultiFileOutput(generatedContent: string): MultiFileOutput | null {
    try {
  ...
  private detectContentType(content: string): { type: string; extension: string } {
  ...
  private formatCode(content: string, type: string): string {
